...
public class BinanceBatchSymbolsWebSocket
{
    ...
    public static Dictionary<string, decimal> _contractSize;
    ...
    public static void Initialize()
    {
        ...
        _contractSize = new Dictionary<string, decimal>();
    }
    ...
    public static async Task ConnectAsync()
    {
        ...
        while (!_cts.IsCancellationRequested) // 循环，直到连接成功
        {
            try
            {
                ...
                {
                    // Console.WriteLine("_currentTradingPairs number = 0 !");
                ...
                var futureSbscriptionPaths = _currentTradingPairs.Select(pair => $"{pair.ToLower()}usd_perp@depth5@100ms");
                ...
                var futureUri = new Uri($"wss://dstream.binance.com/stream?streams={futureFullPath}");

                Console.WriteLine($"_currentTradingPairs number = {_currentTradingPairs.Count}");
                Console.WriteLine(_lastUpdateTime);
            ...
            {
                _reconnectAttempts++;
                // Console.WriteLine($"Connection failed (attempt {_reconnectAttempts}): {ex.Message}");
    ...
    private static void ProcessMessage(string message, string connectionType)
    {
        try
        {
            ...
            else if (connectionType == "FUTURE")
            {
                var datas = JObject.Parse(message);
                var stream = datas["stream"].ToString();
                var symbol = stream.Substring(0, stream.IndexOf('@')).Replace("usd_perp", "").ToUpper();
                var data = datas["data"]; // 解析JSON字符串为对象
    ...
    public static void PrintOrderBook()
    {
        ...
        Console.Clear(); // 清空控制台
        Console.WriteLine($"=== BINANCE 现货-币本位合约监控界面 ===");
        ...
        // 输出精简后的标题行
        Console.Write(AlignString("币种", 6));          // 1. 币种
        Console.Write(AlignString("资费", 15));        // 2. 现货买1价
        Console.Write(AlignString("资费年化", 15));        // 2. 现货买1价

        Console.Write(AlignString("期挂现吃开仓差价", 18));   // 10. 期货挂单现货吃单开仓差价
        Console.Write(AlignString("期挂现吃平仓差价", 18));   // 11. 期货挂单现货吃单平仓差价
        Console.Write(AlignString("现挂期吃开仓差价", 18));   // 12. 现货挂单期货吃单开仓差价
        Console.Write(AlignString("现挂期吃平仓差价", 18));   // 13. 现货挂单期货吃单平仓差价
        Console.Write(AlignString("现吃期吃开仓差价", 18));   // 14. 现货吃单期货吃单开仓差价
        Console.Write(AlignString("现吃期吃平仓差价", 18));   // 15. 现货吃单期货吃单平仓差价

        Console.Write(AlignString("现货买1价", 18));        // 2. 现货买1价
        Console.Write(AlignString("现货买1量", 15));        // 3. 现货买1价数量
        Console.Write(AlignString("现货卖1价", 18));        // 4. 现货卖1价
        Console.Write(AlignString("现货卖1量", 15));        // 5. 现货卖1价数量
        Console.Write(AlignString("期货买1价", 12));        // 6. 期货买1价
        Console.Write(AlignString("期货买1量", 15));        // 7. 期货买1价数量
        ...
        foreach (var item in sortedData)
        {
            ...
            Console.Write(AlignString(symbol, 6));
            Console.Write(AlignString($"{coinFundingRate}%", 15));
            Console.Write(AlignString($"{coinFundingRateYear}%", 15));

            // ... 其余的输出代码保持不变
            Console.Write(AlignString($"{openPercent1:F4}%", 18));
            Console.Write(AlignString($"{closePercent1:F4}%", 18));
            Console.Write(AlignString($"{openPercent2:F4}%", 18));
            Console.Write(AlignString($"{closePercent2:F4}%", 18));
            Console.Write(AlignString($"{openPercent3:F4}%", 18));
            Console.Write(AlignString($"{closePercent3:F4}%", 18));

            Console.Write(AlignString(spotValue.Bid1Price.ToString(), 18));
            ...
            Console.Write(AlignString(spotValue.Ask1Price.ToString(), 18));
            ...
            // Console.Write(AlignString(futureValue.Bid1Quantity.ToString(), 10)); 
            Console.Write(AlignString($"{_contractSize[symbol] * futureValue.Bid1Quantity:F2}u", 15));

            Console.Write(AlignString(futureValue.Ask1Price.ToString(), 12));
            // Console.Write(AlignString(futureValue.Ask1Quantity.ToString(), 10));
            Console.Write(AlignString($"{_contractSize[symbol] * futureValue.Ask1Quantity:F2}u", 12));
    ...
    public static async Task StartConnectionMonitor()
    {
        ...
        {
            Console.WriteLine($"{DateTime.Now:HH:mm:ss} - lost, attempting to reconnect...");
            await ReconnectAsync();
        }
    ...
    public static async Task ConnectAsync_SubscribeToCoinMarginedFundingRates()
    {
        ...
        while (!_cancellationTokenSource.IsCancellationRequested) // 循环，直到连接成功
        {
            try
            {
                ...
                // foreach (var pair in _currentTradingPairs)
                // {
                //     _coinMarginedFundingRates.TryAdd(pair, new decimal());
                // }
                // // 生成每个交易对的订阅路径
                var subscriptionPaths = _currentTradingPairs.Select(pair => $"{pair.ToLower()}usd_perp@markPrice@1s");
                // 组合成完整路径
                string fundingRates = string.Join("/", subscriptionPaths);
                var fundingRatesUri = new Uri($"wss://dstream.binance.com/stream?streams={fundingRates}");

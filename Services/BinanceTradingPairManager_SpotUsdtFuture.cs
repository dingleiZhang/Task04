...
public class BinanceTradingPairManager_SpotUsdtFuture
{
    ...
    {
        while (BinanceBatchSymbolsWebSocket_SpotUsdtFuture._currentTradingPairs.Count == 0)
        {
    ...
    public static async Task UpdateTradingPairsAsync()
    {
        ...
        try
        {
            ...
            if (allPairs.Count > 0)   
            {
                Console.WriteLine($"开始获取交易对信息...");
                Console.WriteLine($"总共可交易对: {allPairs.Count} 个");
            ...
            // 打印最新的前10个交易对
            // PrintTop10TradingPairs(BinanceBatchSymbolsWebSocket_SpotUsdtFuture._currentTradingPairs);
    ...
    private static async Task<HashSet<string>> GetCoinMarginedTradingPairsAsync()
    {
        ...
        try
        {
            ...
            var response = await _httpClient.GetStringAsync(url);
            var exchangeInfo = JsonConvert.DeserializeObject<BinanceCoinMarginedExchangeInfo_SpotUsdtFuture>(response);
            if (exchangeInfo?.Symbols != null)
            {
                foreach (var symbol in exchangeInfo.Symbols)
                {
    ...
    private static void CompareAndPrintTradingPairs(HashSet<string> newPairs)
    {
        lock (_lockObject)
        {
            if (BinanceBatchSymbolsWebSocket_SpotUsdtFuture._currentTradingPairs.Count == 0)
            {
                BinanceBatchSymbolsWebSocket_SpotUsdtFuture._currentTradingPairs = newPairs;
                Console.WriteLine("初始交易对列表已加载");
                return;
            }

            BinanceBatchSymbolsWebSocket_SpotUsdtFuture._removedPairs = new HashSet<string>(BinanceBatchSymbolsWebSocket_SpotUsdtFuture._currentTradingPairs);
            BinanceBatchSymbolsWebSocket_SpotUsdtFuture._removedPairs.ExceptWith(newPairs);

            BinanceBatchSymbolsWebSocket_SpotUsdtFuture._addedPairs = new HashSet<string>(newPairs);
            BinanceBatchSymbolsWebSocket_SpotUsdtFuture._addedPairs.ExceptWith(BinanceBatchSymbolsWebSocket_SpotUsdtFuture._currentTradingPairs);

            // 更新当前交易对列表
            BinanceBatchSymbolsWebSocket._currentTradingPairs = newPairs;
            if (BinanceBatchSymbolsWebSocket_SpotUsdtFuture._removedPairs.Count > 0 || BinanceBatchSymbolsWebSocket_SpotUsdtFuture._addedPairs.Count > 0)
            {
                BinanceBatchSymbolsWebSocket_SpotUsdtFuture.ReconnectAsync(); // 交易队变化，则断开重新订阅
            }
        }
    }
